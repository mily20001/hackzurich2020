...
const ColoringSwitch: React.FC<ColoringSwitchProps> = ({setMode, mode}) => {
  return <Radio.Group           options={Object.values(ColoringMode).map(mode => ({label: mode, value: mode})) as {value: ColoringMode, label:ColoringMode}[]}
                                ...
                                size={'large'}
  />
   // return Object.values(ColoringMode).map(mode =>
}
